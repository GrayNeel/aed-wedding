...
const Dashboard = () => {
  ...
  let loadingSpinner = <div className="spinner-border text-primary" role="status">
              ...
              <CTable align="middle" className="mb-0 border" hover responsive>
                <CTableHead className="text-nowrap">
                  <CTableRow>
                    ...
                    <CTableHeaderCell className="bg-body-tertiary">Nome Invito</CTableHeaderCell>
                    <CTableHeaderCell className="bg-body-tertiary">Stato</CTableHeaderCell>
                    <CTableHeaderCell className="bg-body-tertiary">Numero Invitati</CTableHeaderCell>
                    <CTableHeaderCell className="bg-body-tertiary">Lista Invitati</CTableHeaderCell>
                  ...
                  {invitations.map((item, index) => (
                    <CTableRow v-for="item in invitations" key={index}>
                      <CTableDataCell className="text-center">
                        <CAvatar color="primary" textColor="white">{item.name.charAt(0)}</CAvatar>
                      </CTableDataCell>
                      {/* <CTableDataCell>
                      ...
                      <CTableDataCell>
                        <div>{item.guests.length === 0 ? <em>{'Nessun invitato'}</em> : item.guests.map((guest, index) => (index ? ', ' : '') + guest.name + ' ' + guest.surname)}</div>
                      </CTableDataCell>
                      <CTableDataCell>
                        <CDropdown variant="btn-group">
                          <CButton color="primary" size="sm">Visualizza</CButton>
                          <CDropdownToggle size="sm" color="primary" split/>
                          <CDropdownMenu>
                            <CDropdownItem href="#" onClick={() => API.getQRCode(item.invitationId)}>Genera QR Code</CDropdownItem>
                            <CDropdownItem href="#">Genera Link</CDropdownItem>
                            <CDropdownDivider />
                            <CDropdownItem onClick={() => {setDeleteModalVisible(true); setDeleteInvitation(item)}}>Annulla invito</CDropdownItem>
                          </CDropdownMenu>
                        </CDropdown>
                      </CTableDataCell>
                    </CTableRow>
